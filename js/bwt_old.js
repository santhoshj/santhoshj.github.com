$(document).ready(function(){
    ...
    var ranks = getRanks(suffixes.suffixes);
    var cmap = new CumulativeMap(getCumulativeMap(suffixes.suffixes));

    bwtViewer = new BWTViewer("bwt");
    bwtViewer.load(suffixes.suffixes, suffixes.suffixArray, ranks, cmap);
...
function getSortFunction(key) {
    ...
    return function(a, b) {
        var length = getText(a).length;
        for (var i=0; i < length; ++i) {
            var ac = getText(a)[i], bc = getText(b)[i];
            if (ac === bc)
                continue;
            if (ac === '$' || bc === '$') {
...
function getSuffixes(text) {
    ...
    var sortedSuffixes = suffixes.sort(getSortFunction("text"));
...
function getCumulativeMap(suffixes) {
    var freq = {};
    _.each(suffixes, function (item) {
        var next = item[item.length - 1];
        if (freq[next])
            freq[next] += 1;
        else
            freq[next] = 1;
    ...
    alphabet = alphabet.sort(getSortFunction());
...
CumulativeMap.prototype.getPosition = function(el, elCount) {
    if (this.cumulativeMap[el])
        return this.cumulativeMap[el].start + elCount;
    else
        return -1;
};
